...
function Paddle:init(skin)
    ...
    -- Track to see if player has acquired the key powerup to unlock locked bricks
    self.hasKey = false
end
...
function Paddle:render()
    love.graphics.draw(gTextures['main'], gFrames['paddles'][self.size + 4 * (self.skin - 1)],
        self.x, self.y)
    if self.hasKey then
        love.graphics.draw(gTextures['main'], gFrames['powerups']['key'], 10, VIRTUAL_HEIGHT - 10)
    end
end

-- Simple function to  change the size of the paddle at a certain threshold
-- New size is the integer used to change the size variable. So negative
-- is smaller and positive is larger.
-- 4 is largest size of the paddles and cannot go below 1
function Paddle:changeSize(new_size)
    if self.size < 4 and new_size > 0 then
        self.size = self.size + new_size
        self.width = self.width + 32
    elseif self.size <= 4 and new_size < 0 then
        self.width = self.width - 32
        self.size = self.size + new_size
