    ...
    if self.canInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                -- create copy of highlighted tile so it isn't set to nil in calculateMatches and 
                -- can be used if there is no match (may be changed later)
...
function PlayState:swapBack(newTile)
           -- if no match is found then swap back
    if self.noMatch then
...
function PlayState:ensureSwappable()
